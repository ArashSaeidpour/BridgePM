...
set ColumnCriticalNodeCounter 1
set NumberofEmbededNodes 0
...
if {$BentSectionSwitch==2 && $yCoord<=$GroundElevation} {   
source PileSpringsNodes.tcl;   #### adding pile spring for the embede portions of the pile
source PileSpringsFrictionMaterials.tcl
element zeroLength [expr $startelement]  [expr $k-2] [expr $k-1]  -mat $PileSpringPyMaterial $PileSpringTzMaterial  $PileSpringPyMaterial  -dir 1 2 3
set startelement [expr $startelement+1]
}
...
node $k       $xCoord   $yCoord   $zCoord ;    #### Pile/column body node     
...
set NodeDistanceFromGroundElevation [expr abs($yCoord-$GroundElevation)];   ### This will be used to identify the critical column nodes with maximum curvature and moment

if {$NodeDistanceFromGroundElevation<=5}  {

set ColumnCriticalNode($spancounter,$i,$ColumnCriticalNodeCounter) $k

set ColumnCriticalNodeCounter [expr $ColumnCriticalNodeCounter+1]
...
###### Definign pile spring elements



source PileSpringTipMaterial.tcl
...
set FoundaionMaterial(2) $QzSimpleMaterialForPileTip 
}
...
set k [expr $startelement+$counter] ; #### element counter 
                 ...
                 ####print $fp -ele $k
                 set k [expr $k+1]
                 set ColumnCriticalElementCounter 1 
                                     ...
                                     element	dispBeamColumn	$k	$columnnode($spancounter,$i,$j)	$columnnode($spancounter,$i,[expr $j+1])	4	$BentSec $ColTrans	-mass	$ColMassDen
                                      ...
                                      set YCoordNode1 [nodeCoord $columnnode($spancounter,$i,$j) 2];   ### This will be used to identify the critical column nodes with maximum curvature and moment
                                      set YCoordNode2 [nodeCoord $columnnode($spancounter,$i,[expr $j+1]) 2];   ### This will be used to identify the critical column nodes with maximum curvature and moment

                                      if {[expr abs($YCoordNode1-$GroundElevation)]<=5 || [expr abs($YCoordNode2-$GroundElevation)]<=5}  {

                                      set tempnode [nodeCoord $columnnode($spancounter,$i,$j)] 
                                      puts "first node $columnnode($spancounter,$i,$j)"
                                      puts "second node $columnnode($spancounter,$i,[expr $j+1])"
                                      puts $tempnode
                                      #after 5000
                                      
                                      set ColumnCriticalElement($spancounter,$i,$ColumnCriticalElementCounter) $k

                                      ## set criticalelementnodes [open "$dataDir/Critical column element nodes.txt" w+]

                                      puts $criticalelementnodes "element=$k" 
                                      puts $criticalelementnodes "node 1=$columnnode($spancounter,$i,$j)"
                                      puts $criticalelementnodes "node 1 Coord=[nodeCoord $columnnode($spancounter,$i,$j)] "
                                      puts $criticalelementnodes "node 2=$columnnode($spancounter,$i,[expr $j+1])"
                                      puts $criticalelementnodes "node 2 Coord=[nodeCoord $columnnode($spancounter,$i,[expr $j+1])] "
                                     
                                      set ColumnCriticalElementCounter [expr $ColumnCriticalElementCounter+1]
